...
export default function MainMenuPage() {
  ...
  const fetchMenuOptions = useCallback(async () => {
    try {
      ...
      if (user) {
        ...
        const options = await menuService.getMenuOptionsForUserType(user.userType);
  ...
  const handleOptionSelect = async (optionNumber: number) => {
    ...
    try {
      const option = await menuService.getMenuOptionByNumber(optionNumber);
      ...
      if (option.isAdminOnly && userType !== 'A') {
      ...
      <div className="bg-blue-900 text-white p-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div>
            ...
            <h2 className="text-lg">CardDemo - Main Menu</h2>
          </div>
          ...
          <div className="bg-white shadow-lg rounded-lg p-8">
            <div className="mb-6">
              <div className="text-sm text-gray-600 mb-4">
                <span>Transaction: COMEN</span>
                <span className="ml-4">Program: COMEN01C</span>
              </div>
              <h3 className="text-xl font-bold text-gray-800">Main Menu</h3>
            </div>
            ...
            <div className="mb-6 space-y-2">
              {menuOptions.map((option) => (
                ...
                >
                  ...
                  {option.isAdminOnly && (
                    <span className="ml-auto text-xs text-orange-600 font-semibold">Admin Only</span>
                  )}
                  {option.isComingSoon && (
